...
export default class UnfollowUserService {
  ...
  public async execute({ followerId, followingId } : IRequest): Promise<Omit<User, 'password'>> {
    ...
    if (!isFollowing) throw new AppError('You are already not following this user');

    const { password: _, ...userWithoutPassword } = await this.usersRepository.unfollow({ followerId, followingId });
